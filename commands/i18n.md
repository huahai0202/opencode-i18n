...
1. 如果参数为空，先调用 `i18n-state`，传入 `action: "locales"`，读取当前语言和可用语言；然后使用 `question` 工具让用户图形化选择语言。问题文字必须包含工具返回的 `当前语言: ...`，例如 `选择 OpenCode 界面语言（当前：zh-Hans / 简体中文）`。选项必须只使用语言包里的 `name`，例如 `English`、`简体中文`、`繁體中文`；不要给选项追加“当前”或“默认”。用户选择后调用 `i18n-state`，传入 `action: "locale"` 和用户选择的语言名称。
