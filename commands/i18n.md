...
1. 如果参数为空，先调用 `i18n-state`，传入 `action: "locales"`，读取可用语言；然后使用 `question` 工具让用户图形化选择语言，选项必须使用语言包里的 `name`，例如 `English`、`简体中文`、`繁體中文`；用户选择后调用 `i18n-state`，传入 `action: "locale"` 和用户选择的语言名称。
